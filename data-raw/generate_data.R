...
life.history <- read.csv("data/australia.life.history.csv", na.strings = c("NA","NaN", " ", "", NA)) %>%
  ga.clean.names()
...
trophic.groups <- life.history %>%
  dplyr::mutate(trophic.group = ga.capitalise(rls.trophic.group)) %>%
  dplyr::select(scientific, family, genus, species, trophic.group) %>%
  dplyr::mutate(trophic.group = str_replace_all(.$trophic.group, c("NANA" = "Unknown")))

unique(trophic.groups$trophic.group)

park.popups <- read.csv("data/parks.popups.csv", na.strings = c("NA","NaN", " ", "", NA)) %>%
  ga.clean.names()

zoning <- read.csv("data/zoning.csv", na.strings = c("NA","NaN", " ", "", NA)) %>%
  ga.clean.names()
### ► Life history sheet ----
## Will need to replace with DBCA's own version eventually but this will work for time being
life.history <- read.csv("data/australia.life.history.csv", na.strings = c("NA","NaN", " ", "", NA)) %>%
...
metadata <- list.files(path = data.dir, recursive = T, pattern = "_Metadata.csv", full.names = T) %>% # list all files ending in "_Metadata.csv"
  purrr::map_df(~read.dbca.files_csv(.)) %>% # combine into dataframe
...
points <- list.files(path = data.dir, recursive = T, pattern = "_Points.txt", full.names = T)  %>%
  purrr::map_df(~read.dbca.files_txt(.)) %>%
...
lengths <- list.files(path = data.dir, recursive = T, pattern = "_Lengths.txt", full.names = T)  %>%
  purrr::map_df(~read.dbca.files_txt(.)) %>%
  dplyr::mutate(campaignid = str_replace_all(.$campaignid, c("_Lengths.txt" = ""))) %>%
  dplyr::mutate(number = as.numeric(number)) %>%
  dplyr::mutate(length = as.numeric(length)) %>%
  dplyr::rename(sample = opcode)
...
all.data <- metadata %>%
  ...
  dplyr::mutate(metric = str_replace_all(.$metric, c("total.abundance" = "Total abundance",
                                                     "species.richness" = "Species richness")))

fished.complete.length <- dplyr::semi_join(complete.length, fished.species)

state.mp <- readOGR("spatial/WA_MPA_2018.shp")
...
state.mp$zone <- str_replace_all(state.mp$zone, c(
